...
AlphaMirror 是一个本地优先的推免申请材料管理与打包工具。它面向 2026 届推免申请场景，把院校通知解析、材料清单抽取、已有文件复用、缺漏检查、档案管理、资讯追踪和 PDF/ZIP 导出放在同一个工作台里。
